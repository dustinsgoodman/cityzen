class EsSearch
  ...
  def search(*args)
    ...
    Rails.logger.debug "Model Scope Value: #{@model_scope}"
    Rails.logger.debug "Elastic Search Finalized Query: #{build_search.inspect}, #{options.inspect}"
    
    results = nil
    time = Benchmark.measure {
      results = client.search(build_search, options)
    }
    Rails.logger.debug "ElasticSearch Took #{time}"
